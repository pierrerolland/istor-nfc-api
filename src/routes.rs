...
pub fn routes() -> impl Filter<Extract = impl warp::Reply, Error = Rejection> + Clone {
...
fn get_unassigned_stories() -> impl Filter<Extract = impl warp::Reply, Error = Rejection> + Clone {
    warp::path!("unassigned-stories").and(warp::get()).and_then(handlers::get_unassigned_stories)
}

fn assign_story() -> impl Filter<Extract = impl warp::Reply, Error = Rejection> + Clone {
